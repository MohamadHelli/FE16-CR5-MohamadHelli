...
class Dog extends Animal{
    breed: string;
    training: boolean;  
    constructor(name: string, gender: string, size: string, age: number, image: string, breed:string, training:boolean,vaccine: boolean ){
        super (name, gender, size, age, vaccine, image);
        this.breed = breed;
...
(document.querySelector(".sortdown") as HTMLElement).addEventListener("click", agesort)
function agesort () {
    array.sort(function(min, max) {
        return min.age - max.age
    });
    (document.getElementById("cartanimal") as HTMLElement).innerHTML ="";
    allclasses();
};

(document.querySelector(".sortup") as HTMLElement).addEventListener("click", agesort1)
function agesort1 () {
    array.sort(function(min, max) {
...
function allclasses() {
    ...
    array.forEach((val) => {
        (document.getElementById("cartanimal") as HTMLElement).innerHTML += val.display();
    });
}
